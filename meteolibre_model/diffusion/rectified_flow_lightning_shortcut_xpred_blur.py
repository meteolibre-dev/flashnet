...
def random_gaussian_blur(x, max_kernel=7, max_sigma=2.0, p=0.3):
    """Apply random Gaussian blur to (B, C, T, H, W) tensor with probability p."""
    if torch.rand(1).item() > p:
        return x
    
    # Random kernel size (odd)
    k = torch.randint(3, max_kernel + 1, (1,)).item()
    k = k if k % 2 == 1 else k + 1
    
    # Random sigma
    sigma = torch.empty(1).uniform_(0.5, max_sigma).item()
    ...
    # Reshape for depthwise conv: (C_total, 1, k, k)
    b, c, t, h, w = x.shape
    kernel = kernel_2d.expand(c * t, 1, k, k)
    
    # Reshape to (B, C*T, H, W) for grouped conv
    x_flat = x.reshape(b, c * t, h, w)
    pad = k // 2
    x_blurred = F.conv2d(x_flat, kernel, padding=pad, groups=c * t)
    
    return x_blurred.reshape(b, c, t, h, w)
...
def trainer_step(
    ...
    # progressive noise
    if sigma > 0:
        x_context_aug = random_gaussian_blur(x_context, p=0.4)
        
        # Random noise level per sample, uniform in [0, sigma]
        noise_sigma = torch.rand(b, device=device) * sigma
        noise_sigma = noise_sigma.view(b, 1, 1, 1, 1)
        x_context_t = x_context_aug + noise_sigma * torch.randn_like(x_context)
    else:
